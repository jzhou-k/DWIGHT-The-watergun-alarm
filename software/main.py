...
def cameraMode():
    ...
    def mouseAction(action,x,y, *args): 
        ...
        if(action==cv.EVENT_MOUSEMOVE): 
            ...
            #This now returns a new frame with new coord where the mouse is at
            #empimage = displayCoord(image, x,y)
            #image = tempimage
    ...
    now = datetime.datetime.now()  
    startTime = time.time() 
    waitTime = 60 #waits for 120 seconds, if no face, start shooting at most likely spot for 5 times
    elapsedTime = 0 
    sweep = True
    while True:
        ...
        image = displayCoord(image, mouseCoord[0], mouseCoord[1])
        now = time.time()   
        ...
        if(elapsedTime < waitTime or not sweep): 
            ...
            if(results is not None):
                ...
                moveNshoot(coord[0], coord[1], 1, s)
        else:
            sweep = False 
            record = True 
            s = 1 
